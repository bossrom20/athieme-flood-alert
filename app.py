...
.hero-section {
    background: linear-gradient(135deg, rgba(10,22,40,0.95) 0%, rgba(26,82,118,0.9) 50%, rgba(41,128,185,0.85) 100%);
...
.hero-section::after {
    ...
    right: 0;
    height: 4px;
    background: linear-gradient(90deg, var(--accent-cyan), var(--accent-emerald), var(--accent-gold), var(--accent-cyan));
...
.alert-card.danger {
    background: linear-gradient(145deg, rgba(231,76,60,0.2) 0%, rgba(146,43,33,0.15) 100%);
    border-color: var(--danger);
    box-shadow: var(--shadow-glow-danger);
}

.alert-card.warning {
    background: linear-gradient(145deg, rgba(243,156,18,0.2) 0%, rgba(154,125,10,0.15) 100%);
    border-color: var(--warning);
    box-shadow: var(--shadow-glow-warning);
}

.alert-card.success {
    background: linear-gradient(145deg, rgba(39,174,96,0.2) 0%, rgba(30,132,73,0.15) 100%);
...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, var(--primary-900) 0%, var(--neutral-800) 100%);
...
.footer-section {
    background: linear-gradient(135deg, rgba(10,22,40,0.95) 0%, rgba(22,33,62,0.95) 100%);
    border-radius: var(--radius-xl);
    padding: 2rem;
    margin-top: 3rem;
    border: 1px solid rgba(0,212,255,0.2);
...
# ═══════════════════════════════════════════════════════════════════════════════
# FONCTIONS DE CRÉATION DES GRAPHIQUES (COMPATIBILITÉ PYTHON 3.13)
# ═══════════════════════════════════════════════════════════════════════════════

def create_gauge_chart(probability, risk_level, risk_class):
    """Crée une jauge de probabilité élégante"""
    colors = {'danger': '#e74c3c', 'warning': '#f39c12', 'success': '#27ae60'}
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [0, 100], 'tickwidth': 2, 'tickcolor': '#ffffff', 'tickfont': {'color': '#aab7b8', 'size': 12}},
            ...
            'threshold': {'line': {'color': '#ffffff', 'width': 3}, 'thickness': 0.8, 'value': probability * 100}
        }
    ))
    
    fig.update_layout(
        title=f"<b>Risque {risk_level}</b><br><span style='font-size:14px;color:#aab7b8'>Probabilité d'inondation (24-48h)</span>",
        title_font=dict(size=20, color='#ffffff', family='Space Grotesk'),
        title_x=0.5,
...
def create_historical_chart(df, days=30):
    ...
    # Zone critique (rouge)
    max_y = max(df_recent['Cote_m'].max() + 0.5, SEUIL_CRITIQUE + 0.5)
    fig.add_shape(
        type="rect",
        x0=df_recent['Date'].min(),
        x1=df_recent['Date'].max(),
        y0=SEUIL_CRITIQUE,
        y1=max_y,
    ...
    fig.add_hline(y=SEUIL_ALERTE, line_dash="dash", line_color='#f39c12', line_width=2,
                  annotation_text=f"Seuil alerte ({SEUIL_ALERTE}m)", annotation_position="bottom left")
    fig.add_hline(y=SEUIL_CRITIQUE, line_dash="dash", line_color='#e74c3c', line_width=2,
                  annotation_text=f"Seuil critique ({SEUIL_CRITIQUE}m)", annotation_position="bottom left")
    
    fig.update_layout(
        title=f"📈 Évolution du niveau du fleuve Mono - Derniers {days} jours",
        title_font=dict(size=18, color='#ffffff', family='Space Grotesk'),
        xaxis_title='Date',
        yaxis_title='Cote (mètres)',
        xaxis=dict(gridcolor='rgba(255,255,255,0.05)', tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff')),
        yaxis=dict(gridcolor='rgba(255,255,255,0.05)', tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), range=[0, max_y]),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=450,
...
def create_model_comparison_chart():
    ...
    fig.add_trace(go.Bar(
        ...
        x=metrics,
        y=rf_values,
        marker=dict(color='#00d4ff', line=dict(color='#00d4ff', width=2)),
    ...
    fig.add_trace(go.Bar(
        ...
        x=metrics,
        y=lr_values,
        marker=dict(color='#00d9a5', line=dict(color='#00d9a5', width=2)),
    ...
    fig.update_layout(
        ...
        title_font=dict(size=18, color='#ffffff', family='Space Grotesk'),
        barmode='group',
        xaxis=dict(tickfont=dict(color='#ffffff', size=13), gridcolor='rgba(255,255,255,0.05)'),
        yaxis=dict(title='Score (%)', range=[0, 115], tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)'),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=400,
        margin=dict(l=60, r=30, t=80, b=60),
...
def create_feature_importance_chart():
    ...
    fig = go.Figure(go.Bar(
        ...
        y=features_names,
        orientation='h',
        marker=dict(color=importance, colorscale='Tealgrn', line=dict(color='rgba(255,255,255,0.3)', width=1)),
    ...
    fig.update_layout(
        title='🎯 Importance des Variables Prédictives (Random Forest)',
        title_font=dict(size=18, color='#ffffff', family='Space Grotesk'),
        xaxis=dict(title='Importance relative (%)', tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)', range=[0, 30]),
        yaxis=dict(tickfont=dict(color='#ffffff', size=11), autorange='reversed'),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=450,
...
def create_seasonal_chart(df):
    ...
    fig.add_hline(y=SEUIL_ALERTE, line_dash="dash", line_color='#f39c12', annotation_text="Seuil d'alerte", annotation_position="bottom right")
    
    fig.update_layout(
        title="📅 Saisonnalité des Niveaux (2005-2024) | Rouge = Saison des crues",
        title_font=dict(size=18, color='#ffffff', family='Space Grotesk'),
        xaxis=dict(tickfont=dict(color='#ffffff', size=12), gridcolor='rgba(255,255,255,0.05)'),
        yaxis=dict(title='Cote (mètres)', tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)'),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=400,
        margin=dict(l=60, r=30, t=80, b=60),
...
def create_flood_events_chart(df):
    ...
    fig.add_trace(go.Bar(
        x=events_df['label'],
        y=events_df['cote'],
        marker=dict(color=colors, line=dict(color='rgba(255,255,255,0.3)', width=2)),
    ...
    fig.add_annotation(x='Sept 2007', y=8.88, text='🏆 Record', showarrow=True, arrowhead=2, arrowcolor='#ffd700', font=dict(color='#ffd700', size=12), ax=0, ay=-40)
    
    fig.update_layout(
        title='🌊 Événements d\'Inondation Majeurs (2005-2024)',
        title_font=dict(size=18, color='#ffffff', family='Space Grotesk'),
        xaxis=dict(tickfont=dict(color='#ffffff', size=11), tickangle=-45, gridcolor='rgba(255,255,255,0.05)'),
        yaxis=dict(title='Cote maximale (mètres)', range=[7.5, 9.5], tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)'),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=400,
...
with st.sidebar:
    ...
    date_analyse = st.date_input("📅 Date de l'analyse", value=datetime.now(), help="Date des mesures à analyser")
    
    st.markdown("---")
    st.markdown("#### 💧 Mesures du jour")
    
    cote_actuelle = st.number_input("Cote actuelle (m)", min_value=0.0, max_value=12.0, value=6.5, step=0.01, format="%.2f", help="Niveau d'eau mesuré au limnimètre")
    debit_actuel = st.number_input("Débit actuel (m³/s)", min_value=0.0, max_value=1000.0, value=250.0, step=1.0, format="%.1f", help="Débit calculé à partir de la courbe de tarage")
    
    st.markdown("---")
    st.markdown("#### 📈 Variations observées")
    
    var_cote_24h = st.number_input("Variation 24h (m)", min_value=-3.0, max_value=3.0, value=0.15, step=0.01, format="%.2f", help="Différence de cote avec hier")
    var_cote_48h = st.number_input("Variation 48h (m)", min_value=-5.0, max_value=5.0, value=0.28, step=0.01, format="%.2f", help="Différence de cote avec avant-hier")
    
    st.markdown("---")
    st.markdown("#### 🌧️ Précipitations")
    
    pluie_24h = st.number_input("Pluie dernières 24h (mm)", min_value=0.0, max_value=200.0, value=15.0, step=0.5, help="Cumul pluviométrique des dernières 24 heures")
    pluie_48h = st.number_input("Pluie dernières 48h (mm)", min_value=0.0, max_value=300.0, value=28.0, step=0.5)
    pluie_72h = st.number_input("Pluie dernières 72h (mm)", min_value=0.0, max_value=400.0, value=42.0, step=0.5)
    pluie_7j = st.number_input("Pluie derniers 7 jours (mm)", min_value=0.0, max_value=500.0, value=85.0, step=1.0)
    pluie_14j = st.number_input("Pluie derniers 14 jours (mm)", min_value=0.0, max_value=700.0, value=145.0, step=1.0)
    
    st.markdown("---")
    st.markdown("#### ⚙️ Options")
    
    modele_choisi = st.selectbox("Modèle de prédiction", ["Random Forest (recommandé)", "Régression Logistique", "Moyenne des deux"], help="Sélectionnez le modèle à utiliser pour la prédiction")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["🎯 Prédiction", "📊 Dashboard", "🏆 Performances", "📈 Analyse Historique", "📖 Documentation"])
...
with tab1:
    if analyze_button:
        features = prepare_features(cote_actuelle, debit_actuel, var_cote_24h, var_cote_48h, pluie_24h, pluie_48h, pluie_72h, pluie_7j, pluie_14j)
...
with tab3:
    ...
    with col2:
        ...
        fig_cm_lr.update_layout(
            title='Régression Logistique (6 FP)',
            title_font=dict(color='white', size=16),
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            height=300,
...
with tab4:
    ...
    fig_yearly.add_trace(go.Bar(
        ...
        marker=dict(color=yearly_floods['Jours d\'inondation'], colorscale='Reds', line=dict(color='rgba(255,255,255,0.3)', width=1)),
    ...
    fig_yearly.update_layout(
        xaxis=dict(title='Année', tickmode='linear', tickfont=dict(color='#ffffff'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)'),
        yaxis=dict(title='Nombre de jours', tickfont=dict(color='#aab7b8'), titlefont=dict(color='#ffffff'), gridcolor='rgba(255,255,255,0.05)'),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        height=350,
